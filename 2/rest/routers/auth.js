...
router.post('/login', async function (req, res) {
    ...
    if(!email || !password) {
        return res.status(400).send({ message: 'Nem adtál meg emailcímet vagy jelszót!' });
    }
...
router.post('/register', async function (req, res) {
    ...
    if(!email || !name || !password) {
        return res.status(400).send({ message: 'Nem adtál meg emailcímet, nevet vagy jelszót!' });
    }
